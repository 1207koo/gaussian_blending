    ...
    for idx, relative_path in enumerate(meta["file_path"]):
        ...
        cam_infos.append(
            CameraInfo(
                ...
                width=image.size[0],
                height=image.size[1],
                res=[r for r in res_only if relative_path.endswith(res_name_dict[r])][0],
            )
        )
    return cam_infos
...
def readMultiScaleNerfSyntheticInfo(
    ...
    white_background: bool,
    eval: str,
    train_res: list = [1.0, 2.0, 4.0, 8.0],
) -> SceneInfo:
    print("Reading train from metadata.json")
    train_cam_infos = readMultiScale(
        path, white_background, "train", res_only=train_res
    )
