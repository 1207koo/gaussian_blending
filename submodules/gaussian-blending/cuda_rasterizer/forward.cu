...
__global__ void preprocessCUDA(
    ...
    // 2D covariance and opacity neatly pack into one float4
...
renderCUDA(
    ...
    float T_val = 1.0f; // accumulated transmittance. t_i * l_i,1 * l_i,2 in the paper
    float2 T_xy = pixf;
    ...
    if (inside) {
        ...
        for (int ch = 0; ch < CHANNELS; ch++)
        {
            final_color[ch * H * W + pix_id] = C[ch] + T_val * bg_color[ch];
            out_color[ch * H * W + pix_id] = C[ch] + T_val * bg_color[ch];
        }
    }
}
